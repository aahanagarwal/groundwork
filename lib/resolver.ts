...
function emptySite(slug: string, label: string, inputAddress: string): SiteRecord {
  return {
    id: slug,
    slug,
    label,
 ...
 */
export function wktToGeoJson(wkt: string | null | undefined): GeoJSON.Geometry | null {
  if (!wkt) return null;
...
export async function resolveSite(
  ...
  const lookup = await mireye.lookup({ input: address }, { agent: "resolver", siteId: slug });

  // --- No key / no fixture: seeded fallback, clearly flagged ---------------
  if (!lookup.ok) {
    if (demo && (lookup.refusal.code === "no_api_key" || lookup.refusal.code === "fixture_missing")) {
  ...
  if (data.disposition === "no_match") {
    return refuse({
      ...
      retryable: false,
      hint: data.hint ?? "Check the street name, and include the city and state.",
    });
  }
