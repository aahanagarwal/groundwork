...
export function advertisingModule(
  ...
  const closureDriver = attribution.drivers.find((d) => d.kind === "road_closure");
  ...
  return {
    ...
    payload: {
      ...
      campaign: {
        ...
        spend_cap_cents: Math.round((closure ? wastedSpend : windowSpend) * 100),
      },
      targeting: {
        geo_locations: {
          custom_locations: [
            {
              ...
              polygon: tradeArea.polygonGeoJson.coordinates[0].map(([lng, lat]) => ({
