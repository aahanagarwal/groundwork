...
const SO_WHAT: Record<EventKind, string> = {
  ...
  holiday: "Expected. Plan staffing around it rather than reading it as a trend.",
  fuel_price:
...
export function buildInsight(
  ...
  const grouped = groupByKind(drivers, toCustomers, basketSizeUsd, grossMarginPct);
  ...
  const streetShare = totalMovement > 0 ? Math.min(1, explainedPoints / totalMovement) : 0;
  const unknownShare =
    totalMovement > 0
  ...
  return {
    ...
    unexplainedMarginUsd:
      toCustomers(attribution.unexplainedPoints) * basketSizeUsd * grossMarginPct,
    drivers: grouped,
...
function decideVerdict(input: {
  ...
  const { attribution: a, streetShare, unknownShare, biggest, customersDelta } = input;
