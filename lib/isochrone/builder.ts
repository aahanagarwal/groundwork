...
export async function buildIsochrone(
  ...
  const bearings: BearingResult[] = bearingDegs.map((deg, b) => {
    const samples = [...samplesByBearing[b]].sort(
      (x, y) => x.distanceMi - y.distanceMi,
    );
  ...
  if (refine) {
    ...
    if (refinable.length > 0) {
      const pass2Targets = refinable.map((br) => {
        ...
        const scaled =
          outer.distanceMi * (minutes / Math.max(outer.durationMinutes, 0.5));
      ...
      const pass2 = await probe(
        ...
        refinable.map((br, i) =>
          destinationPoint(
            origin,
            br.bearingDeg,
            pass2Targets[i] * METERS_PER_MILE,
          ),
        ),
        minutes,
      );
      ...
      if (pass2.ok) {
        refinable.forEach((br, i) => {
          ...
          br.errorMinutes =
            measured === null ? null : Math.abs(measured - minutes);
          br.flag = "refined";
          ...
          // crossing we have.
          const resorted = [...br.samples].sort(
            (x, y) => x.distanceMi - y.distanceMi,
          );
  ...
  const ring = bearings.map((br) => {
    const p = destinationPoint(
      origin,
      br.bearingDeg,
      br.boundaryDistanceMi * METERS_PER_MILE,
    );
    return [p.lng, p.lat];
  });
  ring.push(ring[0]);
  ...
  return ok(
    {
      ...
      method:
        ...
        (refine
          ? `, then re-measured with one probe per bearing at the interpolated distance.`
          : `.`),
      notes,
    },
    {
...
export function interpolateBoundary(
  ...
  if (inside.length === 0) {
    ...
    const scale = Math.min(
      1,
      targetMinutes / Math.max(nearest.durationMinutes, 0.1),
    );
  ...
  const beyond = usable.find(
    (s) =>
      s.distanceMi > outermostInside.distanceMi &&
      s.durationMinutes > targetMinutes,
  );

  if (!beyond) {
    return {
      distanceMi: outermostInside.distanceMi,
      flag: "truncated_outward",
    };
  }

  const span = beyond.durationMinutes - outermostInside.durationMinutes;
  const t =
    span <= 0 ? 0 : (targetMinutes - outermostInside.durationMinutes) / span;
  return {
    distanceMi:
      outermostInside.distanceMi +
      t * (beyond.distanceMi - outermostInside.distanceMi),
    flag: "interpolated",
  };
}
