...
function toRefusal(
  ...
  const hints: Record<string, string> = {
    ...
    unresolvable_input:
      "None of the locators in this request could be resolved.",
  ...
  return {
    ...
    message:
      detail?.message ??
      `Mireye ${endpoint} returned ${status} with no message body.`,
...
function provenanceFor(endpoint: string, cached: boolean): Provenance {
  ...
  return {
    ...
    mocked: false,
    note: cached
      ? "Served from a recorded response — no credits spent."
      : undefined,
  };
}
...
async function call<T>(
  ...
  if (!config.mireye.apiKey) {
    const refusal: Refusal = {
      code: "no_api_key",
      message:
        "MIREYE_API_KEY is not set, so the ground layer cannot be reached.",
      retryable: false,
  ...
  try {
    ...
    const refusal: Refusal = {
      code: "network_error",
      message:
        error instanceof Error ? error.message : "The request never completed.",
      retryable: true,
    };
    record({
      ...base,
  ...
  record({
    ...
    creditsActual:
      paidDrivingCalcs !== null
        ? Math.max(5, paidDrivingCalcs * 12)
        : creditsEstimated,
    durationMs,
    httpStatus: response.status,
    refused: false,
...
export const mireye = {
  ...
  laborShed(
    op: Omit<LaborShedOp, "op">,
    opts: Omit<CallOptions, "creditsEstimated">,
  ) {
    return mireye.proximity<LaborShedResponse>(
      { op: "labor_shed", ...op },
      opts,
    );
  },
