...
export function estimateLookupCredits(
...
export function estimateFetchCredits(fields: string[], plan = "build"): number {
  ...
  return (
    fields.length * CREDITS_PER_FETCH_FIELD +
    (touchesParcel ? resolveCredits(plan) : 0)
  );
}
...
export function estimateProximityCredits(op: ProximityOp): CreditEstimate {
  ...
  const addressLocators = locators.filter(
    (l) => !isCoordinateLocator(l),
  ).length;
  const drivingPart = drivingCalcs * CREDITS_PER_DRIVING_CALC;
  const floorApplied = drivingPart < floor;
  const credits =
    Math.max(floor, drivingPart) +
    addressLocators * CREDITS_PER_ADDRESS_LOCATOR;
