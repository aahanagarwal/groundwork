...
export interface Normals {
  ...
  byDayOfYear: Record<
    string,
    { apparentMaxMean: number; apparentMaxSd: number; precipMean: number; precipSd: number }
  >;
  yearsCovered: number;
}
...
export async function fetchNormals(at: LatLng): Promise<SourceResult<Normals>> {
  ...
  const history = await archive(at, NORMALS_START, NORMALS_END, "1991-2020-raw", "normals-raw");
...
export function scoreAnomalies(
  ...
  return observed.map((row) => {
    ...
    const heatAnomalyC =
      row.apparentMaxC === null ? null : row.apparentMaxC - norm.apparentMaxMean;
    const precipitationAnomalyMm =
      row.precipitationMm === null ? null : row.precipitationMm - norm.precipMean;
    return {
      ...row,
