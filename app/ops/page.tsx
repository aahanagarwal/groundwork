...
export default async function OpsPage() {
  ...
  return (
    <>
      <Sheet>
        <Plate no="OPS" name="Instrumentation" of="not the owner-facing product" />

        <p className="max-w-[68ch] text-[17px] leading-snug">
          This page is aimed at a technical reader, not at a shop owner. It reports
          what the system spent, what it refused, and how much of what it claims is
        ...
        <section className="mt-8 card p-5">
          ...
          {isMockMode() ? (
            <p className="mt-4 border-l-2 border-signal pl-3 text-[15px] leading-snug">
              ...
              fixtures where they exist and <strong>refuse</strong> where they do
              not; the trade area falls back to the local OSRM router and says so on
              its own citation. Weather is live regardless - Open-Meteo needs no key.
        ...
        <section className="mt-6">
          ...
          <div className="grid grid-cols-2 gap-px border-[1.5px] border-ink bg-rule sm:grid-cols-4">
            ...
            <Stat
              ...
              value={totalCalls ? `${((totalCacheHits / totalCalls) * 100).toFixed(0)}%` : "-"}
            />
            <Stat label="Credits spent" value={totalCredits.toLocaleString()} />
            <Stat
              label="Refusal rate"
              value={totalCalls ? `${((totalRefusals / totalCalls) * 100).toFixed(0)}%` : "-"}
            />
          </div>
                ...
                {[...byEndpoint.entries()]
                  ...
                  .map(([endpoint, r]) => (
                    <tr key={endpoint} className="border-b border-rule">
                      <td className="py-2 font-mono text-[12.5px]">{endpoint}</td>
                      <Td>{r.calls}</Td>
        ...
        <section className="mt-8">
          ...
          <p className="mb-3 max-w-[68ch] text-[15px] leading-snug">
            Each agent asks the Budget Broker before any live call. Threat Watch has
            the tightest ceiling on purpose: it runs unattended, and nothing that
            runs while nobody is watching gets a large budget.
          </p>
              ...
              <div key={b.agent} className="border border-rule px-4 py-2.5">
                ...
                <div className="mt-1.5 h-1.5 w-full bg-limestone">
                  <div
                    ...
                    style={{ width: `${Math.min(100, (b.spent / b.ceiling) * 100)}%` }}
                  />
                </div>
        ...
        <section className="mt-8">
          ...
          <p className="mb-3 max-w-[68ch] text-[15px] leading-snug">
            Mireye returns no polygon geometry - confirmed against its live OpenAPI
            spec. These boundaries are built by us from measured drive times, and
            each carries its own error figure: after interpolating a bearing&rsquo;s
            boundary we re-probe at exactly that point and compare the drive time
            found there against the {TRADE_AREA_MINUTES}-minute target.
          </p>
          {polygons.length > 0 ? (
              ...
              <tbody>
                {polygons.map((t) => (
                  <tr key={t.id} className="border-b border-rule">
                    ...
                    <Td>{((1 - t.areaSqMi / t.naiveAreaSqMi) * 100).toFixed(0)}%</Td>
        ...
        <section className="mt-8 card p-5">
          ...
          <ul className="space-y-2 text-[15px] leading-snug">
            <li>
              <strong>No language model computes a number.</strong> Attribution is
              deterministic code. The narrator receives the finished object and
              turns it into sentences; it cannot assert a figure that was not
              already in that object.
            </li>
            <li>
              <strong>Baseline</strong> is a trailing 28-day level times a
              day-of-week factor, fitted only on days when nothing was happening on
              the street. A baseline fitted through a closure absorbs the closure.
            </li>
            <li>
              <strong>Drivers enter simultaneously</strong> in one ridge-regularised
              regression of daily residuals on driver magnitude - not sequential
              subtraction, so what each driver gets is its partial effect with the
              others held constant.
            </li>
            <li>
              <strong>Shrinkage is deliberate,</strong> at {RIDGE_FRACTION} of the
              design&rsquo;s mean diagonal. Plain least squares on indicator
              regressors forces residuals to sum to zero within each driver&rsquo;s
              own day-set, so when the window being explained <em>is</em> that
              day-set, the drivers account for exactly 100% as an accounting
              identity and ε comes out at 0.0 whatever really happened. Shrunk-away
              weight falls through to ε rather than being handed to whichever driver
              was active.
            </li>
            <li>
              <strong>Saturated windows are flagged,</strong> not smoothed over. If
              every day in the window had something happening, the total is solid
              and the split between drivers is an estimate - and the brief says so
              in those words.
            </li>
          </ul>
        </section>
        ...
        <section className="mt-8">
          ...
          <div className="grid grid-cols-2 gap-px border-[1.5px] border-ink bg-rule sm:grid-cols-4">
            ...
            <Stat
              label="Approved"
              value={String(decisions.filter((d) => d.status === "approved").length)}
            />
          ...
          <p className="mt-3 max-w-[68ch] text-[15px] leading-snug">
            ...
            place an order, or publish anything. Approving marks a draft approved in
            the database and stops.
          </p>
        </section>
        ...
        {refusalCodes.length > 0 ? (
          <section className="mt-8">
            ...
            <p className="mb-3 max-w-[68ch] text-[15px] leading-snug">
              A refusal is a designed state, not an error. A high rate here with an
              absent key is the system working correctly - declining to guess.
            </p>
...
function Td({ children }: { children: React.ReactNode }) {
  return <td className="py-2 text-right font-mono text-[12.5px] tabular">{children}</td>;
}
