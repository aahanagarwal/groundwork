...
async function main() {
  ...
  console.log(
    "Fetching 1991-2020 normals (first run downloads ~11k days, then caches)...",
  );

  const normals = await fetchNormals(at);
  if (!normals.ok) {
    console.log("REFUSED:", normals.refusal);
    return;
  }
  console.log(
    `normals: ${Object.keys(normals.data.byDayOfYear).length} calendar days, ${normals.data.yearsCovered} years`,
  );
  const jul09 = normals.data.byDayOfYear["07-09"];
  console.log(
    `  9 July normal: apparent max ${jul09.apparentMaxMean.toFixed(1)}C (sd ${jul09.apparentMaxSd.toFixed(1)}), precip ${jul09.precipMean.toFixed(2)}mm`,
  );

  const observed = await fetchObserved(at, "2026-05-19", "2026-08-16");
  if (!observed.ok) {
    console.log("REFUSED:", observed.refusal);
    return;
  }
  console.log(
    `observed: ${observed.data.length} days  [${observed.provenance.source}]`,
  );
  ...
  for (const e of events) {
    console.log(
      `  ${e.startDate}${e.endDate !== e.startDate ? ` -> ${e.endDate}` : "          "}  ${e.kind.padEnd(5)} mag ${e.magnitude.toFixed(2)}  ${e.label}`,
    );
  }

  const hottest = [...scored].sort(
    (a, b) => (b.heatZ ?? -99) - (a.heatZ ?? -99),
  )[0];
  console.log(
  ...
  const wettest = [...scored].sort(
    (a, b) => (b.precipitationMm ?? 0) - (a.precipitationMm ?? 0),
  )[0];
  console.log(
    `wettest:            ${wettest.date}  ${wettest.precipitationMm}mm vs normal ${wettest.normalPrecipitationMm?.toFixed(2)}mm`,
  );
}
void main();
