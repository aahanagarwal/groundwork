...
async function doResolve() {
  ...
  for (const site of DEMO_SITES) {
    ...
    console.log(
      `    ${s.lat}, ${s.lng}   confidence ${s.confidence ?? "—"}   ${s.matchMethod ?? ""}`,
    );
    console.log(
      `    ${s.county ?? "?"} · tract ${s.tractGeoid ?? "?"} · ${s.cbsaName ?? "?"}`,
    );
    console.log(
      `    flood zone ${s.femaFloodZone ?? "—"} · elevation ${s.elevationM ?? "—"}m · ${s.timezone ?? "—"}`,
    );
    console.log(
      `    parcel ${s.parcelId ?? "none"}${s.parcelAreaM2 ? ` · ${Math.round(s.parcelAreaM2).toLocaleString()} m²` : ""}${s.parcelZoning ? ` · zoning ${s.parcelZoning}` : ""}`,
    );
    console.log(
      `    boundary geometry: ${s.parcelGeoJson ? "yes" : "no"}   approximate: ${s.approximate}`,
    );
    if (s.countyMarket) {
      const m = s.countyMarket;
      console.log(
        `    county market: pop ${m.population?.toLocaleString() ?? "?"} · income $${m.median_household_income_usd?.toLocaleString() ?? "?"} · employment ${m.employment_yoy_pct ?? "?"}% YoY`,
      );
    }
...
async function doIsochrone(slug: string) {
  ...
  if (!site?.lat || !site?.lng) {
    console.log(
      `No resolved coordinate for "${slug}". Run the resolve step first.`,
    );
    return;
  }

  console.log(
    `Building the ${TRADE_AREA_MINUTES}-minute trade area for ${site.label} from real Mireye drive times.`,
  );
  ...
  const result = await buildIsochrone(
    ...
    mireyeProbe("isochrone", site.id),
    {
      ...
      engineLabel: "Mireye /v1/proximity (op: screen)",
    },
  );
  ...
  console.log(
    `    area ${iso.areaSqMi.toFixed(2)} mi² vs ${iso.naiveAreaSqMi.toFixed(0)} mi² circle (${((1 - iso.areaSqMi / iso.naiveAreaSqMi) * 100).toFixed(0)}% smaller)`,
  );
  console.log(`    paid driving calcs: ${iso.paidDrivingCalcs}`);
  console.log(
    `    CREDITS  estimated 960  ·  actual ${spent}  ·  delta ${spent - 960 >= 0 ? "+" : ""}${spent - 960}`,
  );
  console.log(
    `    boundary accuracy: mean ±${iso.accuracy.meanErrorMinutes.toFixed(2)} min, worst ±${iso.accuracy.maxErrorMinutes.toFixed(2)} min`,
  );
  const radii = iso.bearings.map((b) => b.boundaryDistanceMi);
  console.log(
    `    reach: ${Math.min(...radii).toFixed(2)}–${Math.max(...radii).toFixed(2)} mi  (anisotropy ${(Math.max(...radii) / Math.max(Math.min(...radii), 0.01)).toFixed(1)}x)`,
  );
...
async function doIou() {
  console.log(
    "PHASE 4 VALIDATION — local OSRM counterfactual vs the Mireye-derived polygon.\n",
  );
  console.log(
    "Same builder, same probe grid, same interpolation on both sides, so any",
  );
  console.log("difference is the road-network model, not the algorithm.\n");

  for (const site of sites.all()) {
    const mine = tradeAreas.find(
      (t) => t.siteId === site.id && t.engine === "mireye",
    );
    if (!mine || !site.lat || !site.lng) continue;

    let osrm = tradeAreas.find(
      (t) => t.siteId === site.id && t.engine === "osrm",
    );
    if (!osrm) {
      ...
      if (!built.ok) {
        console.log(
          `${site.label}: local router unavailable (${built.refusal.code})`,
        );
        continue;
      }
      osrm = {
        ...
        id: `${site.id}-8-osrm`,
        engine: "osrm",
        polygonGeoJson: built.data.geojson,
        areaSqMi: built.data.areaSqMi,
      } as TradeAreaRecord;
      tradeAreas.put(osrm);
    }
    ...
    console.log(
      `    Mireye ${mine.areaSqMi.toFixed(2)} mi²   OSRM ${osrm.areaSqMi.toFixed(2)} mi²`,
    );
    console.log(
      `    IoU ${cmp.iou.toFixed(3)}   (intersection ${cmp.intersectionSqMi.toFixed(2)} / union ${cmp.unionSqMi.toFixed(2)} mi²)\n`,
    );
  }
}
...
async function main() {
  ...
  if (mode === "resolve") await doResolve();
    ...
    for (const s of only ? [only] : DEMO_SITES.map((d) => d.slug))
      await doIsochrone(s);
