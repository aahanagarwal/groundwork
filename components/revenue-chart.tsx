...
export function RevenueChart({
  ...
  const max =
    Math.max(...series.map((p) => Math.max(p.tickets, p.baseline))) * 1.05;
  const min =
    Math.min(...series.map((p) => Math.min(p.tickets, p.baseline))) * 0.9;
  ...
  const line = (get: (p: (typeof series)[number]) => number) =>
    series
      .map(
        (p, i) =>
          `${i === 0 ? "M" : "L"}${x(i).toFixed(1)},${y(get(p)).toFixed(1)}`,
      )
      .join(" ");
    ...
    <div>
      <div className="mb-2 flex flex-wrap items-baseline justify-between gap-3">
        ...
        <span className="font-mono text-[11.5px] text-survey">
          {active ? (
            <>
              {active.date} · {active.tickets} actual ·{" "}
              {Math.round(active.baseline)} expected ·{" "}
              <span
                className={active.residual < 0 ? "text-ultra" : "text-survey"}>
                {active.residual > 0 ? "+" : ""}
      ...
      <div className="mt-1.5 flex flex-wrap gap-4 font-mono text-[11px] text-survey">
        ...
        <span className="flex items-center gap-1.5">
          <span className="inline-block h-0 w-4 border-t-[1.5px] border-dashed border-survey" />{" "}
          expected
        </span>
        <span className="flex items-center gap-1.5">
          <span className="inline-block h-2.5 w-4 bg-signal/50" /> the window
          being explained
        </span>
      </div>
    </div>
