...
Magnitude is the normalised anomaly, capped at 1. It means "how much of this driver was present" — _not_ "how bad was it for the shop". That second question is what the fitted coefficient answers, and conflating them is how you end up asserting your own assumptions back at yourself.
...
**Bands.** 95% intervals from the coefficient standard errors, scaled by how much of the driver fell inside the window. These are the SEs of the _shrunk_ estimator: they describe where this estimator would land on repeated samples and do not include the bias the shrinkage deliberately introduces. Stated here rather than buried.
