...
`max_credits` is sent on every call. Mireye checks it *before* charging the driving matrix, so an accidental 500-probe request returns a priced 422 rather than a bill.
...
Rate limits: V1 has no metered request quotas - the constraint is credits, which is what the broker governs. Timeouts: 130 s for `/v1/ask` (server-bounded at 110 s; the docs are explicit that a short client timeout aborts requests that keep running *and billing*), 70 s for `/v1/fetch`.
